...
def preprocess_image(img):
    """
    Converts image to grayscale, enhances contrast, and thresholds using adaptive method.
    Returns original and binary thresholded image.
    """
    ...
    # Adaptive thresholding for better bubble isolation
...
def detect_bubbles(thresh):
    ...
    for cnt in contours:
        if len(cnt) >= 5:
            ...
            if 4 < radius < 25:  # Expanded range for bubble size
                bubbles.append((int(x), int(y), int(radius)))

    print(f"🟢 Bubble detection: {len(bubbles)} bubbles found")
    return bubbles

def map_bubbles_to_responses(bubbles, question_grid, radius_tolerance=25):
    """
            ...
            for bubble in bubbles:
                if len(bubble) == 3:
                    ...
                    if dist < radius_tolerance:
                        responses[q + 1] = chr(65 + idx)  # A, B, C, D
                        break
                else:
